# run_adaptive_tau_safe_mode_demo.py
import numpy as np
...
from adaptive_tau_policy import AdaptiveTauConfig, compute_self_trust, adaptive_tau_from_trust
...
def main():
    ...
    # Adaptive tau request
    at_cfg = AdaptiveTauConfig(tau_min=0.70, tau_max=0.95, local_radius=0)
    S = compute_self_trust(I_bottle, start, at_cfg)
    tau_req = adaptive_tau_from_trust(S, at_cfg)

    print(f"Start={start}, Goal={goal}")
    print(f"I(start)={I_bottle[start]:.3f} -> trust S={S:.3f}")
    print(f"Adaptive requested tau={tau_req:.3f} (range [{at_cfg.tau_min}, {at_cfg.tau_max}])")
