# ================================================================
# Experimental Comparison:
# Επίδραση διαφορετικού Dropout Probability
#
# Συγκρίνουμε:
#   dropout = 0.1
#   dropout = 0.3
#   dropout = 0.5
#
# Για κάθε run μετράμε:
#   - Baseline MSE
#   - Online MSE (before)
#   - Online MSE (after)
#   - Avg VAR (before)
#   - Avg VAR (after)
# ================================================================

import os
import numpy as np
import pandas as pd
import torch

from online_drift_uncertainty_exp import OnlineDriftUncertaintyAdapterExp

FEATURE_COLS = ["entropy", "local_uncertainty", "speed"]
TARGET_COL = "drift"


def load_drift_dataset(csv_path: str):
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"Dataset not found at {csv_path}")

    df = pd.read_csv(csv_path)

    missing = [c for c in FEATURE_COLS if c not in df.columns]
    if missing:
        raise ValueError(f"Missing feature columns: {missing}. Columns: {list(df.columns)}")

    if TARGET_COL not in df.columns:
        raise ValueError(f"Target column '{TARGET_COL}' not found. Columns: {list(df.columns)}")

    X = df[FEATURE_COLS].values.astype(np.float32)
    y = df[TARGET_COL].values.astype(np.float32)

    return X, y


def mse(a, b) -> float:
    return float(np.mean((a - b) ** 2))


def run_dropout_experiment(X, y, dropout_p, device, seed=0):
    """
    Τρέχει ένα πλήρες πείραμα online learning
    για δεδομένο dropout probability
    και επιστρέφει metrics.
    """

    np.random.seed(seed)
    torch.manual_seed(seed)

    N = X.shape[0]
    N0 = int(0.4 * N)

    X_base = X[:N0]
    y_base = y[:N0]
    X_online = X[N0:]
    y_online = y[N0:]

    adapter = OnlineDriftUncertaintyAdapterExp(
        model_path=None,           # πάντα από scratch για δίκαιη σύγκριση
        input_dim=X.shape[1],
        lr=1e-4,
        device=device,
        max_buffer_size=512,
        weight_decay=1e-6,
        dropout_p=dropout_p,
    )

    # ================= Baseline =================
    y_pred_base = adapter.predict(X_base).reshape(-1)
    base_mse = mse(y_pred_base, y_base)

    # ================= Online =================
    mse_before = []
    mse_after = []
    var_before = []
    var_after = []

    window = 64

    for i in range(X_online.shape[0]):
        x_i = X_online[i]
        y_i = y_online[i]

        mean_b, var_b = adapter.predict_with_uncertainty(x_i, n_samples=20)
        pred_b = mean_b.reshape(-1)[0]
        var_b_scalar = var_b.reshape(-1)[0]

        adapter.add_observation(x_i, np.array(y_i, dtype=np.float32))

        if (i + 1) % 8 == 0:
            loss = adapter.online_update(batch_size=32)
        else:
            loss = 0.0

        mean_a, var_a = adapter.predict_with_uncertainty(x_i, n_samples=20)
        pred_a = mean_a.reshape(-1)[0]
        var_a_scalar = var_a.reshape(-1)[0]

        mse_before.append((pred_b - y_i) ** 2)
        mse_after.append((pred_a - y_i) ** 2)
        var_before.append(var_b_scalar)
        var_after.append(var_a_scalar)

        if (i + 1) % 100 == 0:
            mb = float(np.mean(mse_before[-window:]))
            ma = float(np.mean(mse_after[-window:]))
            vb = float(np.mean(var_before[-window:]))
            va = float(np.mean(var_after[-window:]))
            print(
                f"[DROP={dropout_p} STEP {i+1}/{X_online.shape[0]}] "
                f"rolling MSE before={mb:.6f} | after={ma:.6f} | "
                f"VAR before={vb:.6f} | VAR after={va:.6f} | last loss={loss:.6f}"
            )

    return {
        "dropout": dropout_p,
        "base_mse": base_mse,
        "online_mse_before": float(np.mean(mse_before)),
        "online_mse_after": float(np.mean(mse_after)),
        "online_var_before": float(np.mean(var_before)),
        "online_var_after": float(np.mean(var_after)),
    }


def main():
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    csv_path = os.path.join(base_dir, "drift_dataset.csv")

    print(f"[DRP-INFO] Loading dataset from: {csv_path}")
    X, y = load_drift_dataset(csv_path)
    print(f"[DRP-INFO] Loaded dataset: {X.shape[0]} samples, {X.shape[1]} features")

    if X.shape[0] < 200:
        print("[DRP-WARN] Πολύ λίγα samples για ουσιαστικό online πείραμα.")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[DRP-INFO] Using device: {device}")

    dropout_values = [0.1, 0.3, 0.5]
    results = []

    for dp in dropout_values:
        print(f"\n[DRP-INFO] Running experiment with dropout={dp}")
        metrics = run_dropout_experiment(X, y, dp, device=device, seed=0)
        results.append(metrics)

    # ================= Summary Table =================
    print("\n================ DROPOUT COMPARISON SUMMARY ================")
    print("Dropout | Baseline MSE | Online MSE (before) | Online MSE (after) | Avg VAR (before) | Avg VAR (after)")
    print("--------+--------------+---------------------+---------------------+------------------+-----------------")

    for m in results:
        print(
            f"{m['dropout']:^7} | "
            f"{m['base_mse']:.6f}   | "
            f"{m['online_mse_before']:.6f}          | "
            f"{m['online_mse_after']:.6f}         | "
            f"{m['online_var_before']:.6f}       | "
            f"{m['online_var_after']:.6f}"
        )

    print("============================================================")


if __name__ == "__main__":
    main()
