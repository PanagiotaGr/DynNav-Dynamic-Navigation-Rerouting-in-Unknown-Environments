# ================================================================
# Online Drift Uncertainty Learning with Neural Network + MC Dropout
# ================================================================
...
# ---------------------------------------------------------------
# Μικρό MLP για drift regression με Dropout (για epistemic uncertainty)
# ---------------------------------------------------------------
class SimpleDriftUncertaintyNet(nn.Module):
    def __init__(
        self,
        input_dim: int,
...
# ---------------------------------------------------------------
# Online Adaptation Adapter
# ---------------------------------------------------------------
class OnlineDriftUncertaintyAdapter:
    """
    Μηχανισμός online προσαρμογής για drift uncertainty learning.
    Διατηρεί buffer παρατηρήσεων και βελτιώνει το μοντέλο σταδιακά.
    """
    ...
    def __init__(
        ...
        # -----------------------------------------------------
        # Προσπάθεια φόρτωσης προϋπάρχοντος μοντέλου
        # -----------------------------------------------------
            ...
            if isinstance(state, nn.Module):
                ...
                if "model_state_dict" in state and isinstance(state["model_state_dict"], dict):
                    try:
                        ...
                        print(f"[WARN] Failed to load model_state_dict: {e}")

                if self.model is None:
                    try:
                        net.load_state_dict(state)
                        self.model = net
                        print("[INFO] Loaded plain state_dict from file.")
                    except Exception as e:
                        print(f"[WARN] Failed to load state_dict: {e}")

        # -----------------------------------------------------
        # Αν δεν βρέθηκε μοντέλο, δημιουργούμε νέο
        # -----------------------------------------------------
        if self.model is None:
            print(
                "[WARN] No compatible pretrained model found. "
                "Initializing SimpleDriftUncertaintyNet from scratch."
            )
            self.model = SimpleDriftUncertaintyNet(
                input_dim=input_dim,
                hidden_dims=hidden_dims,
                dropout_p=dropout_p
            )
    ...
    def save_model(self, path: str) -> None:
        ...
        print(f"[INFO] Saved model state_dict to: {path}")
